...
def initialize(job):
    with job:
        ...
        il_conc=job.statepoint()['concIL'] # mol/L , concIL is the concentration of either cation or anion
        acn_conc=job.statepoint()['concacn'] 
        pack_dim=6
        packing_box = mb.Box([pack_dim,pack_dim,pack_dim])
        unit_n=constants.Avogadro/(10**(24)) # mol/L to  number of molecules / nm^3
        ...
        if n_ions == 0: 
            ...
            system = mb.fill_box(compound=[cation, anion], n_compounds=[n_ions,n_ions], box=packing_box)
            ilPM = Forcefield(get_ff_path(kpl))
            il_system= ilPM.apply(il)
            il_system.save('{}_tfsi.top'.format(job.statepoint()['cation']), combine='all', overwrite=True)
            system.save('{}_tfsi.gro'.format(job.statepoint()['cation']), combine='all',overwrite=True, residues=[job.statepoint()['cation'], 'tfsi'])
        else:
            system = mb.fill_box(compound=[acn,  cation, anion], n_compounds=[n_acn,n_ions,n_ions], box=packing_box)
            ...
            print("Saving .gro, .pdb and .top ... ")
            system_total.save('system.top', combine='all', overwrite=True) #for gromacs
            system.save('system.gro', combine='all',overwrite=True, residues=[job.statepoint()['cation'], 'tfsi', 'acn'])
...
def run_rdf(job):
    ...
    if os.path.exists(os.path.join(job.workspace(), 'com.gro')):
        ...
        selections['cation'] = trj.topology.select('resname {}'.format(job.statepoint()['cation']))

...
concentration = [
    ...
    8.5,
    10,
]  # x mol IL in 24.36 mol acn

x = Symbol("x")
ES_conc = (
    []
)  # [[IL mol/L, acn mol/L],,,,] for emim_tfsi in acn at different concentration
BS_conc = []
HS_conc = []
for IL_1mol_V in [E_1mol_V, B_1mol_V, H_1mol_V]:
    ...
    for con_n in range(len(concentration)):
        if concentration[con_n] == 0:
            ...
            s = solve(
                x * IL_1mol_V + x * 24.36 / concentration[con_n] * acn_1mol_V - 1, x
            )
            IL_conc.append(
                [s[0], s[0] * 24.36 / concentration[con_n]]
            )  # IL_conc[[IL conc, acn conc], ....]
